        ...
        trainset = ImageFolderTruncated(
            ...
            transform=transforms.Compose(
                [
                    ...
                    transforms.Lambda(
                        lambda x: F.pad(
                            Variable(x.unsqueeze(0), requires_grad=False), (4, 4, 4, 4), mode="reflect"
                        ).data.squeeze()
                    ),
...
def load_poisoned_dataset(args):
    ...
    if args.dataset in ("mnist", "emnist"):
        ...
        vanilla_test_loader = torch.utils.data.DataLoader(
            emnist_test_dataset, batch_size=args.test_batch_size, shuffle=False, **kwargs
        )
        targetted_task_test_loader = torch.utils.data.DataLoader(
            fashion_mnist_test_dataset, batch_size=args.test_batch_size, shuffle=False, **kwargs
        )
        ...
        if args.poison_type == "ardis":
            ...
            targetted_task_test_loader = torch.utils.data.DataLoader(
                ardis_test_dataset, batch_size=args.test_batch_size, shuffle=False, **kwargs
            )
        ...
        if args.poison_type == "southwest":
            ...
            if args.attack_case == "edge-case":
                with open(
                    "./../../../data/edge_case_examples/southwest_cifar10/southwest_images_new_train.pkl", "rb"
                ) as train_f:
                    saved_southwest_dataset_train = pickle.load(train_f)

                with open(
                    "./../../../data/edge_case_examples/southwest_cifar10/southwest_images_new_test.pkl", "rb"
                ) as test_f:
            ...
            if args.attack_case == "edge-case":
                ...
                samped_poisoned_data_indices = np.random.choice(
                    saved_southwest_dataset_train.shape[0], num_sampled_poisoned_data_points, replace=False
                )
                ...
                num_sampled_poisoned_data_points = 100  # N
                samped_poisoned_data_indices = np.random.choice(784, num_sampled_poisoned_data_points, replace=False)
            ######################################################
            ...
            logging.info("!!!!!!!!!!!Num clean data points in the mixed dataset: {}".format(num_sampled_data_points))
            ...
            poisoned_trainset = CIFAR10_Poisoned(
                ...
                poisoned_indices=np.arange(
                    ...
                    tempt_poisoned_trainset.shape[0] + saved_southwest_dataset_train.shape[0],
                ),
                train=True,
                download=True,
            ...
            remaining_indices = [
                i for i in cifar10_whole_range if i not in sampled_indices_train + sampled_indices_test
            ]
            logging.info(
                "!!!!!!!!!!!Num poisoned data points in the mixed dataset: {}".format(
            ...
            with open(
                "./../../../data/edge_case_examples/greencar_cifar10/green_car_transformed_test.pkl", "rb"
            ) as test_f:
            ...
            logging.info("Poisoned Trainset Shape: {}".format(poisoned_trainset.data.shape))
            logging.info("Poisoned Train Target Shape:{}".format(poisoned_trainset.targets.shape))
            ...
            logging.info("Backdoor (Green car) test-data shape we collected: {}".format(saved_new_green_cars_test.shape))
            sampled_targets_array_test = 2 * np.ones(
